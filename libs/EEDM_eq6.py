...
#     readObj.data(i)
#     print("rank = ", rank, " time = ", readObj.time)
...
for i in range(slt,elt):
    ...
    # Compute the polytropic k, where dk/dt|_Lagrangian = 0
    poly_k = p / rho**gamma
    ...
    print("rank = ", rank, " Memory used before derivatives = ", mem, " Gb")
    ...
    print("rank = ", rank, " Memory used before speeds = ", mem, " Gb")
    ...
    hdfnew.create_dataset('Divx', data=np.array(divx,dtype='float64'), compression='gzip', compression_opts=9)
    hdfnew.create_dataset('Divy', data=np.array(divy,dtype='float64'), compression='gzip', compression_opts=9)
    hdfnew.create_dataset('Divz', data=np.array(divz,dtype='float64'), compression='gzip', compression_opts=9)
    if pre.divBCond:
        hdfnew.create_dataset('Div1', data=np.array(div1,dtype='float64'), compression='gzip', compression_opts=9)
        del(div1)
    ...
    if g == 0:
        ...
        hdfnew.create_dataset('G', data=np.array(- rho * g * vz,dtype='float64'), compression='gzip', compression_opts=9)
    
    #**********************************************************************************************************
    hdfnew.create_dataset('Entx', data=np.array(Entx,dtype='float64'), compression='gzip', compression_opts=9)
    hdfnew.create_dataset('Enty', data=np.array(Enty,dtype='float64'), compression='gzip', compression_opts=9)
    hdfnew.create_dataset('Entz', data=np.array(Entz,dtype='float64'), compression='gzip', compression_opts=9)
    del(Entx,Enty,Entz)
    ...
    print("rank = ", rank, " Memory used before Alfven = ", mem, " Gb")
    ...
    hdfnew.create_dataset('alf_x_1', data=np.array(alfx1,dtype='float64'), compression='gzip', compression_opts=9)
    del(alfx1)
    ...
    hdfnew.create_dataset('alf_x_2', data=np.array(alfx2,dtype='float64'), compression='gzip', compression_opts=9)
    del(alfx2)
    ...
    if len(alphasx[mask]) > 0:
        ...
        print("Setting the negative elements of alphasx to zero!")
        alphasx[mask] = 0
    ...
    if len(alphafx[mask]) > 0:
        ...
        print("Setting the negative elements of alphafx to zero!")
        alphafx[mask] = 0
    ...
    print("rank = ", rank, " Memory used after Slow before cleaning slowx1 and slowx2 = ", mem, " Gb")
    ...
    if pre.mode == "XYZUpDownSeparated":
        hdfnew.create_dataset('slo_x_1', data=np.array(slowx1,dtype='float64'), compression='gzip', compression_opts=9)
        hdfnew.create_dataset('slo_x_2', data=np.array(slowx2,dtype='float64'), compression='gzip', compression_opts=9)
    elif pre.mode == "UpDownCombined":
        hdfnew.create_dataset('slo_x', data=np.array(slowx1 + slowx2,dtype='float64'), compression='gzip', compression_opts=9)
    ...
    if pre.mode == "XYZUpDownSeparated":
        hdfnew.create_dataset('fas_x_1', data=np.array(fastx1,dtype='float64'), compression='gzip', compression_opts=9)
        hdfnew.create_dataset('fas_x_2', data=np.array(fastx2,dtype='float64'), compression='gzip', compression_opts=9)
    elif pre.mode == "UpDownCombined":
        hdfnew.create_dataset('fas_x', data=np.array(fastx1 + fastx2,dtype='float64'), compression='gzip', compression_opts=9)
    ...
    if len(alphasy[mask]) > 0:
        ...
        print("Setting the negative elements of alphasy to zero!")
        alphasy[mask] = 0
    ...
    if len(alphafy[mask]) > 0:
        ...
        print("Setting the negative elements of alphafy to zero!")
        alphafy[mask] = 0
    ...
    hdfnew.create_dataset('alf_y_1', data=np.array(alfy1,dtype='float64'), compression='gzip', compression_opts=9)
    del(alfy1)
    ...
    hdfnew.create_dataset('alf_y_2', data=np.array(alfy2,dtype='float64'), compression='gzip', compression_opts=9)
    del(alfy2)
    ...
    if pre.mode == "XYZUpDownSeparated":
        hdfnew.create_dataset('slo_y_1', data=np.array(slowy1,dtype='float64'), compression='gzip', compression_opts=9)
        hdfnew.create_dataset('slo_y_2', data=np.array(slowy2,dtype='float64'), compression='gzip', compression_opts=9)
    elif pre.mode == "UpDownCombined":
        hdfnew.create_dataset('slo_y', data=np.array(slowy1 + slowy2,dtype='float64'), compression='gzip', compression_opts=9)
    ...
    2*(ax**2 + az**2)*c*alphasy*(-1 + gamma) - 2*csy*(ax*vx + az*vz)*alphasy*(-1 + gamma)*sy)*(ax*BxDy*c*alphasy*rho + az*BzDy*c*alphasy*rho + np.sqrt(ax**2 + az**2)*
    pDy*alphafy*sqmurho + np.sqrt(ax**2 + az**2)*cfy*vyDy*alphafy*rho*sqmurho - csy*(ax*vxDy + az*vzDy)*alphasy*rho*sqmurho*sy))/((ax**2 + az**2)*c**2*(-1 + gamma)*sqmurho))
    ...
    if pre.mode == "XYZUpDownSeparated":
        hdfnew.create_dataset('fas_y_1', data=np.array(fasty1,dtype='float64'), compression='gzip', compression_opts=9)
        hdfnew.create_dataset('fas_y_2', data=np.array(fasty2,dtype='float64'), compression='gzip', compression_opts=9)
    elif pre.mode == "UpDownCombined":
        hdfnew.create_dataset('fas_y', data=np.array(fasty1 + fasty2,dtype='float64'), compression='gzip', compression_opts=9)
    ...
    if len(alphasz[mask]) > 0:
        ...
        print("Setting the negative elements of alphasz to zero!")
        alphasz[mask] = 0
    ...
    if len(alphafz[mask]) > 0:
        ...
        print("Setting the negative elements of alphafz to zero!")
        alphafz[mask] = 0
    ...
    hdfnew.create_dataset('alf_z_1', data=np.array(alfz1,dtype='float64'), compression='gzip', compression_opts=9)
    del(alfz1)
    ...
    hdfnew.create_dataset('alf_z_2', data=np.array(alfz2,dtype='float64'), compression='gzip', compression_opts=9)
    del(alfz2)
    ...
    (ax*alphafz*rho*(BxDz*c + cfz*vxDz*sqmurho*sz))/np.sqrt(ax**2 + ay**2) + (ay*alphafz*rho*(ByDz*c + cfz*vyDz*sqmurho*sz))/np.sqrt(ax**2 + ay**2)))/(2.*c**2*sqmurho))
    ...
    if pre.mode == "XYZUpDownSeparated":
        hdfnew.create_dataset('slo_z_1', data=np.array(slowz1,dtype='float64'), compression='gzip', compression_opts=9)
        hdfnew.create_dataset('slo_z_2', data=np.array(slowz2,dtype='float64'), compression='gzip', compression_opts=9)
    elif pre.mode == "UpDownCombined":
        hdfnew.create_dataset('slo_z', data=np.array(slowz1 + slowz2,dtype='float64'), compression='gzip', compression_opts=9)
    ...
    if pre.mode == "XYZUpDownSeparated":
        hdfnew.create_dataset('fas_z_1', data=np.array(fastz1,dtype='float64'), compression='gzip', compression_opts=9)
        hdfnew.create_dataset('fas_z_2', data=np.array(fastz2,dtype='float64'), compression='gzip', compression_opts=9)
    elif pre.mode == "UpDownCombined":
        hdfnew.create_dataset('fas_z', data=np.array(fastz1 + fastz2,dtype='float64'), compression='gzip', compression_opts=9)
